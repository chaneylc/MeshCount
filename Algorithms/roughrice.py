...
def findGroundTruths(img):
    ...
    return groundTruths, groundTruthConvexity
...
if __name__ == "__main__":
    ...
    groundTruths, groundTruthConvexity = findGroundTruths(img)
   ...
   # gradient = cv2.morphologyEx(gradient, cv2.MORPH_CLOSE, kernel, iterations=1)
    ...
    for index, c in enumerate(contours):
        ...
        if hierarchy[0][index][2] == -1 and cv2.contourArea(c) > 100:
            hull = cv2.convexHull(c, True)
            polyDP = cv2.approxPolyDP(c, 0.01 * cv2.arcLength(c, True), True)
            convexity = float(len(polyDP))/ len(hull)
            print(groundTruthConvexity)
            contour_list.append(convexity)
            if convexity <= 1.5:
                floorCount += 1
