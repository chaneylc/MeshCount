...
if __name__ == "__main__":
   ...
   # gradient = cv2.morphologyEx(gradient, cv2.MORPH_CLOSE, kernel, iterations=1)
    ...
    #markersA = cv2.morphologyEx(markers, cv2.MORPH_CLOSE, np.ones((3,3)), iterations = 1)
    #markersA[borders == -1] = 0
        ...
        if area < 500000:
            for i, hier in enumerate(hierarchy[0]):
                if hier[3] == index:
                    ...
                    #children.append((i, cv2.contourArea(contours[i])))
                    if childArea > gtAvgArea*.5:
                        new_parents.append(contours[i])
                        #new_parents.append(contours[index])
        #children_area = sum(map(lambda x: x[1], children))


        # if children_area > 0.8 * area and children_area < area * 1.1:
    ...
    for index, c in enumerate(new_parents):
        ...
        g = rng.randint(0,128)
        b = rng.randint(0,128)
        cv2.namedWindow('mask.jpg', cv2.WINDOW_NORMAL)
        color = (r, g, b)


        if area < 500000: #and area > 500:
            # children = []
            # for i, hier in enumerate(hierarchy[0]):
            #     if hier[3]  == index:
            #         children.append(cv2.contourArea(contours[i]))

            # if sum(children) > 0.8 * area and sum(children) < area * 1.1:
            ...
            #     for i, hier in enumerate(hierarchy[0]):
            #         if hier[3]  == index:
            #             cv2.drawContours(img, contours, i, [150, 255, 150], 1)
